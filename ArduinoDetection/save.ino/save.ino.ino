...
void sendEmail(){
  ...
  smtpData.setMessage("<div style=\"color:#2f4468;\"><h1>Somoene is here !</h1><p>- Sent from Luca's control space</p></div>", true);
...
const char IRSENSOR_page[] PROGMEM = R"=====(
<!DOCTYPE html>
<html>
<link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css" integrity="sha384-JcKb8q3iqJ61gNV9KGb8thSsNjpSL0n8PARn9HuZOnIxN0hoP+VmmDGMN5t9UJ0Z" crossorigin="anonymous">
<style>
...
<body>
<h1 class="text-secondary">IR Sensor</h1>
<div class="container">
  <div class="row">
    <div class="col-sm text-center">
      <p class="text-center" id="valueA"></p>
    </div>
    <div class="col-sm text-center">
      <p class="text-center" id="valueD"></p>
    </div>
  </div>
</div>
...
function getData() {
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      document.getElementById("valueA").innerHTML = this.responseText;
    }
  };
  xhttp.open("GET", "irSensorValueA", true);
  xhttp.send();
}
...
void handle_irSensor() {
  Serial.println("IR Sensor Page");
  String m = IRSENSOR_page; //Read HTML contents
  server.send(200, "text/html", m); //Send web page
}

void handle_irSensorValueA() {
  Serial.println("IR Sensor Value");
  server.send(200, "text/plane",String(IRvalueA)); 
}

void handle_irSensorValueD() {
  Serial.println("IR Sensor Value");
  server.send(200, "text/plane",String(IRvalueD)); 
}
...
void setup(void) {
  ...
  server.on("/irSensor", handle_irSensor);
  server.on("/irSensorValueA", handle_irSensorValueA);
  server.on("/irSensorValueD", handle_irSensorValueD);
